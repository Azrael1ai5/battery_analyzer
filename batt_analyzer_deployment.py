...
def create_line_graph_plotly(simulation_results, min_soc_percent):
    ...
    # Add other traces to secondary y-axis
    ...
    # Update layout
    fig.update_layout(
        title_text='System Performance Over 24 Hours (Grid-Tied)',
        xaxis_title='Hour of Day',
        # --- MODIFIED LEGEND POSITION ---
        legend=dict(
            yanchor="top",     # Anchor legend from its top edge
            y=1.02,            # Position the top slightly above the plot area (adjust 1.02 as needed)
    ...
    fig.update_yaxes(title_text="<b>State of Charge (%)</b>", secondary_y=False, range=[0, 100]) # Removed tickformat, range 0-100
